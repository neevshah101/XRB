...
!    vals(9) = MoI
  ...
  integer function extras_finish_step(id)
   ...
   !     s% max_model_number = 9900000 ! increase this
        if ((s% center_h1 < TAMS_h1_threshold) .and. (s% center_he4 < 1.0d-4)) then ! .and. (s% center_c12 < 2.0d-2)) then
           write(*,'(g0)') "Single star depleted helium, terminating from run_star_extras"
