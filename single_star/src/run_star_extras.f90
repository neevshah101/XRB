...
contains
  ...
  subroutine extras_controls(id, ierr)
    ...
    !s% other_wind => BJ_NL_Decin
    s% other_wind => BJ_NL_dJ
...
!    vals(9) = MoI
  ...
  integer function extras_finish_step(id)
    ...
    real(dp) :: envelope_mass_fraction, L_He, L_tot, min_center_h1_for_diff, &
         critmass, feh, rot_full_off, rot_full_on, frac2, TAMS_h1_threshold, he4_threshold, c12_threshold
    ...
    TAMS_h1_threshold = 1d-2
    he4_threshold = 1d-2
    c12_threshold = 1d-2

    ! Added by Neev to save TAMS, helium depletion, carbon depletion data
        ! find donor's TAMS
    if ((s% lxtra(1) .eqv. .false.) .and. &
       (s% xa(s%net_iso(ih1), s% nz) < TAMS_h1_threshold)) then
       s% lxtra(1) = .true.
       s% xtra(1) = s% r(1)
       print *, "saved single star radius at TAMS", s% xtra(1)/Rsun
       write(fname, fmt="(a14)") 'single_star_TAMS.mod'
       call star_write_model(id, fname, ierr)
       write(fname, fmt="(a15)") 'single_star_TAMS.data'
       call star_write_profile_info(id, trim(s% log_directory)//'/'//trim(fname), ierr)
    end if

    if ((s% lxtra(17) .eqv. .false.) .and. &
       (s% xa(s%net_iso(ihe4), s% nz) < he4_threshold)) then
       s% lxtra(17) = .true.
       s% xtra(1) = s% r(1)
       print *, "saved single radius at helium depletion", s% xtra(1)/Rsun
       write(fname, fmt="(a14)") 'single_star_he_depletion.mod'
       call star_write_model(id, fname, ierr)
       write(fname, fmt="(a15)") 'single_star_he_depletiom.data'
       call star_write_profile_info(id, trim(s% log_directory)//'/'//trim(fname), ierr)
    end if

    if ((s% lxtra(18) .eqv. .false.) .and. (s% xa(s%net_iso(ic12), s% nz) < c12_threshold) .and. (s% xa(s%net_iso(ihe4), s% nz) < he4_threshold)) then
       s% lxtra(18) = .true.
       s% xtra(1) = s% r(1)
       print *, "saved single star radius at carbon depletion", s% xtra(1)/Rsun
       write(fname, fmt="(a14)") 'single_star_c_depletion.mod'
       call star_write_model(id, fname, ierr)
       write(fname, fmt="(a15)") 'single_star_c_depletiom.data'
       call star_write_profile_info(id, trim(s% log_directory)//'/'//trim(fname), ierr)
    end if
    ...
    !if ((s% center_h1 < TAMS_h1_threshold) .and. (s% center_he4 < 5.0d-2) .and. (s% center_c12 < 5.0d-2)) then
   ...
   !     if ((s% center_h1 < TAMS_h1_threshold) .and. (s% center_he4 < 1.0d-4) .and. (s% center_c12 < 2.0d-2)) then
   !        write(*,'(g0)') "Single star depleted carbon, terminating from run_star_extras"
   !        extras_finish_step = terminate
   !     endif
   !     ! if ((s% center_h1 < TAMS_h1_threshold) .and. (s% center_he4 < 1.0d-4) .and. ((s% surface_he4 > 0.3) .or. (s% surface_c12 > 0.1))) then
   ...
   !  endif
    ...
    if ((s% center_h1 < TAMS_h1_threshold) .and. (s% initial_mass <= 0.6d0) .and. (s% star_age > 5.0d10) )then
       termination_code_str(t_xtra2) = 'central H1 mass fraction below TAMS_h1_threshold'
  ...
  subroutine extras_after_evolve(id, ierr)
   ...
   !subroutine BJ_NL_Decin(id, Lsurf, Msurf, Rsurf, Tsurf, X, Y, Z, w, ierr)
subroutine BJ_NL_dJ(id, Lsurf, Msurf, Rsurf, Tsurf, X, Y, Z, w, ierr)
   use star_def
   type (star_info), pointer :: s
   integer, intent(in) :: id
...
contains
   ...
   subroutine eval_highT(w5)
      ...
      if (X < 0.4d0) then
         w5 = 2*1d-11 * pow(L1/Lsun, 1.29d0) * pow(Y, 1.7d0) * sqrt(Z)
         !print*, "Nugis & Lamers"
      else
...
!end subroutine BJ_NL_Decin
end subroutine BJ_NL_dJ
